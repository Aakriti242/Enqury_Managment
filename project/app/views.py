...
def loginuser(request):
    if request.method=="POST":
        ...
        try:
            ...
            if user:
                if user.Password==pw:
                    return render(request,'userdashboard.html',{'email':email,'pass':pw})
                else:
                    msg="wrong password"
                    return render(request,'loginuser.html',{'msg':msg})
        except:
...
def query(request):
   email=request.POST['email']
   return render(request,'queryform.html',{'email':email})
...
def insertquery(request):
    if request.method=="POST":
        ...
        data=queryDatabase.objects.filter(Email=em)
